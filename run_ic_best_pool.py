"""
Run the 700 IC-best BCL STDP simulations as ONE Slurm job with a process pool,
...
Usage (inside an salloc/sbatch allocation):
    ...
    python run_ic_best_pool.py --workers 60 --dry-run
    python run_ic_best_pool.py --workers 60 --skip-existing
"""
...
# gen-4 best from optimizer_ion_channels/checkpoint_edges.pkl — identical to
# the FIT_ARGS block in submit_ic_best_700.sh.
FIT_ARGS = [
...
def _worker(task):
    ...
    pair, proto, wd = task
    t0 = time.time()
    log_path = os.path.join(wd, f"icb_pool_{proto}_{pair}.log")
