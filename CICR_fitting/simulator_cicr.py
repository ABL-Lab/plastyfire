...
import re
import logging
import multiprocessing  # Keeping just in case other modules expect it, though unused
import numpy as np
import pandas as pd
...
SYNREC = ["rho_GB", "Use_GB", "gmax_AMPA", "cai_CR", "vsyn", "ica_NMDA", "ica_VDCC", "effcai_GB", "prime_CICR", "ca_cicr"]
...
def runsinglecell(sim_config, post_gid, stimulus, node_pop, fixhp):
    ...
    # Use isolated subprocess instead of multiprocessing to avoid NEURON DLL load conflicts
    import tempfile, pickle, subprocess, sys, os
    with tempfile.NamedTemporaryFile(suffix=".pkl", delete=False) as f_in, \
        ...
        pickle.dump((sim_config, post_gid, stimulus, {}, node_pop, fixhp), f_in)
        in_name, out_name = f_in.name, f_out.name

    script = f'''
import pickle, sys, os
sys.path.insert(0, r"{os.path.dirname(os.path.abspath(__file__))}")
from simulator_cicr import _runsinglecell_proc
with open("{in_name}", "rb") as f:
    args_list = list(pickle.load(f))
results = args_list[3]  # This is the empty dict passed for results
_runsinglecell_proc(*args_list)
...
def _set_local_params(synapse, fit_params, extra_params, c_pre=0., c_post=0.):
    ...
    if fit_params is not None:  # update thresholds
        ...
        if all(key in fit_params for key in ["a10", "a11"]) and extra_params["loc"] == "basal":
            ...
            raw_theta_p = fit_params["a10"] * c_pre + fit_params["a11"] * c_post
            synapse.hsynapse.theta_p_GB = max(raw_theta_p, synapse.hsynapse.theta_d_GB + 0.01)
        ...
        if all(key in fit_params for key in ["a30", "a31"]) and extra_params["loc"] == "apical":
            ...
            raw_theta_p = fit_params["a30"] * c_pre + fit_params["a31"] * c_post
            synapse.hsynapse.theta_p_GB = max(raw_theta_p, synapse.hsynapse.theta_d_GB + 0.01)
...
def c_pre_finder(sim_config, fit_params, syn_extra_params, pre_gid, post_gid,
    ...
    # Use perfectly isolated subprocess to avoid NEURON DLL double load
    import tempfile, pickle, subprocess, sys
    with tempfile.NamedTemporaryFile(suffix=".pkl", delete=False) as f_in, \
         tempfile.NamedTemporaryFile(suffix=".pkl", delete=False) as f_out:
        pickle.dump((sim_config, fit_params, syn_extra_params, pre_gid, post_gid, node_pop, edge_pop, fixhp), f_in)
        in_name, out_name = f_in.name, f_out.name

    script = f'''
import pickle, sys, os
sys.path.insert(0, r"{os.path.dirname(os.path.abspath(__file__))}")
from simulator_cicr import _c_pre_finder_process
with open("{in_name}", "rb") as f:
    args = pickle.load(f)
results = _c_pre_finder_process(*args)
with open("{out_name}", "wb") as f:
    pickle.dump(results, f)
'''
    subprocess.run([sys.executable, "-c", script], check=True)
...
def c_post_finder(sim_config, fit_params, syn_extra_params, pre_gid, post_gid, stimulus,
    ...
    with tempfile.NamedTemporaryFile(suffix=".pkl", delete=False) as f_in, \
         tempfile.NamedTemporaryFile(suffix=".pkl", delete=False) as f_out:
        pickle.dump((sim_config, fit_params, syn_extra_params, pre_gid, post_gid, stimulus, node_pop, edge_pop, fixhp), f_in)
        in_name, out_name = f_in.name, f_out.name

    script = f'''
import pickle, sys, os
sys.path.insert(0, r"{os.path.dirname(os.path.abspath(__file__))}")
from simulator_cicr import _c_post_finder_process
with open("{in_name}", "rb") as f:
    args = pickle.load(f)
results = _c_post_finder_process(*args)
with open("{out_name}", "wb") as f:
    pickle.dump(results, f)
'''
    subprocess.run([sys.executable, "-c", script], check=True)
...
def runconnectedpair(workdir, fit_params=None, syn_rec_lst=None, fastforward=None,
    ...
    with tempfile.NamedTemporaryFile(suffix=".pkl", delete=False) as f_in, \
         tempfile.NamedTemporaryFile(suffix=".pkl", delete=False) as f_out:
        # None for manager and log_queue as they are not picklable, worker will adapt
        pickle.dump((workdir, fit_params, syn_extra_params, pre_gid, post_gid, t_end,
                     c_pre, c_post, syn_rec_lst, fastforward, node_pop, edge_pop, fixhp), f_in)
        in_name, out_name = f_in.name, f_out.name

    script = f'''
import pickle, sys, os
sys.path.insert(0, r"{os.path.dirname(os.path.abspath(__file__))}")
from simulator_cicr import _runconnectedpair_prefire_process
with open("{in_name}", "rb") as f:
    args = pickle.load(f)
results = {{}}
# Unpack args and pass empty results and None for log_queue
_runconnectedpair_prefire_process(results, *args, None)
with open("{out_name}", "wb") as f:
