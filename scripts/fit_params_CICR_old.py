...
FIT_PARAMS = [
    ...
    ("ca_event_thresh", 0.0001, 0.002),    # cai threshold for detecting a spike (mM)
    ("a_er", 0.0, 5.0),                    # cpre coeff for ER-loading threshold
    ("b_er", 0.0, 5.0),                    # cpost coeff for ER-loading threshold
...
def _apply_cicr_batch(cai, t, c_pre, c_post,
                      tau_mGluR, mGluR_thresh, ca_event_thresh,
    ...
    fe = 0.17
    caAvg = 0.0017      # mM
    MAX_DT_SUB = 0.1    # ms, max substep for ER dynamics stability
    ...
    for s in range(n_syn):
        ...
        for i in range(n_time - 1):
            ...
            is_above = 1 if cai[s, i] > ca_event_thresh else 0
            ...
            for _ in range(n_sub):
                cai_now = cai[s, i] + ca_cicr
                cai_um = 1000.0 * cai_now

                # SERCA (Hill, active when cai_now > gamma_er)
                if cai_now > gamma_er and cai_um > 0.0:
                    J_serca = g_serca * (cai_um * cai_um) / (K_serca * K_serca + cai_um * cai_um)
                else:
                    J_serca = 0.0

                # CICR release (gate ON and ER has calcium above cai)
                ...
                # update ER (conservation: fc*dcai + fe*dca_er = 0)
                ca_er = ca_er - (fc / fe) * J_net * dt_sub
                if ca_er < 0.0:
                    ca_er = 0.0

                # update cytoplasmic CICR contribution
...
def objective(x):
    ...
    Parameter vector layout (25 params):
      x[0:2]   gamma_d, gamma_p
      x[2:6]   a00, a01, a02, d0       (basal theta_d)
      x[6:10]  a10, a11, a12, p0       (basal theta_p)
      x[10:14] a20, a21, a22, d0_ap    (apical theta_d)
      x[14:18] a30, a31, a32, p0_ap    (apical theta_p)
      x[18:25] CICR params
    """
...
def main():
    ...
    _compute_pair_ratio(
        ...
        3.0, np.array([4.0]),
        100.0, 200.0,
        1.0, 2.0, 0.0, 0.0,   # basal theta_d: a00, a01, a02, d0
        1.0, 2.0, 0.0, 0.0,   # basal theta_p: a10, a11, a12, p0
        1.0, 2.0, 0.0, 0.0,   # apical theta_d: a20, a21, a22, d0_ap
        5.0, 2.0, 0.0, 0.0,   # apical theta_p: a30, a31, a32, p0_ap
    )
    _apply_cicr_batch(
        _dummy_cai, _dummy_t, np.zeros(1), np.zeros(1),
        500.0, 3.0, 0.0003, 1.0, 1.0, 1.9565, 0.5,
    )
