#!/usr/bin/env python3
"""
Parameter fitting for Graupner-Brunel plasticity model with Dual-Hill CICR.

Optimizes 10 GB params + 12 CICR params. Features Analog Calcium Amplitude 
tracking to preserve biological fidelity (synaptic failures, STP).
"""
...
# Total 22 Parameters (10 GB + 12 CICR)
FIT_PARAMS = [
    ...
    ("a30", 1.0, 10.0),
    ("a31", 1.0, 5.0),
    # Analog Dual-Hill Primed CICR parameters
    ("tau_IP3", 100.0, 5000.0),            # IP3 priming leaky integrator (ms)
    ("K_P", 0.5, 10.0),                    # Priming Hill threshold (analog units)
    ("n_prime", 1.0, 8.0),                 # Priming Hill steepness
    ("a_evt", 0.0, 0.05),                  # pre coeff for analog amplitude baseline
    ("b_evt", 0.0, 0.05),                  # post coeff for analog amplitude baseline
    ("a_er", 0.0, 0.05),                   # pre coeff for SERCA threshold
    ("b_er", 0.0, 0.05),                   # post coeff for SERCA threshold
    ("g_serca_cicr", 0.01, 50.0),          # SERCA max rate
    ("g_release_cicr", 0.001, 10.0),       # CICR max release rate
    ("sat_cap", 0.001, 0.5),               # Saturation cap for calcium amplitude (mM)
    ("K_Ca", 0.0001, 0.005),               # Trigger Ca Hill threshold (mM)
    ("m_ca", 1.0, 8.0),                    # Trigger Ca Hill steepness
]
...
def _apply_cicr_batch(cai, t, c_pre, c_post,
    ...
    """
    Apply Primed CICR using Analog Calcium Amplitude Tracking and Dual-Hill Gates.
    """
    K_serca = 0.1       
    fc = 0.83
    fe = 0.17
    ...
    for s in range(n_syn):
        ...
        for i in range(n_time - 1):
            ...
            # --- 1. Analog Calcium Amplitude Tracking (Key 1) ---
            # Driven PURELY by the shape and magnitude of the raw calcium trace.
            ...
            if is_above == 1 and was_above == 0:
                ...
                # Falling edge: compute fractional analog amplitude
            ...
            # Leaky integration of the analog signal
            ...
            # Fast Trigger Gate (Evaluated strictly on RAW NMDAR/VDCC influx)
...
def _compute_rho_final(effcai, t, theta_d, theta_p, gamma_d, gamma_p, rho0):
    ...
    for i in range(n - 1):
        ...
        rho_new = rho + dt * drho
        if rho_new < 0.0: rho_new = 0.0
        elif rho_new > 1.0: rho_new = 1.0
        rho = rho_new
...
def _compute_pair_ratio(effcai, t, c_pre, c_post, is_apical, rho0,
                        ...
                        gamma_d, gamma_p, a00, a01, a10, a11, a20, a21, a30, a31):
    ...
    for p_idx, (proto, target_ratio) in enumerate(_TARGETS.items()):
        ...
        for pd_item in _PROTOCOL_DATA[proto]:
            ...
            ratio = _compute_pair_ratio(
                effcai, pd_item["t"], pd_item["c_pre"], pd_item["c_post"],
...
def _compute_ratios_cicr(x):
    # Unpack identically to objective
    gamma_d, gamma_p = x[0], x[1]
    a00, a01 = x[2], x[3]
    a10, a11 = x[4], x[5]
    a20, a21 = x[6], x[7]
...
def train_surrogate_cicr(X, Y, seed=42):
    """Train sklearn MLPRegressor surrogate: params (22) -> ratios (5)."""
...
def run_nn(n_samples=50000, workers=1, n_starts=500, seed=42,
    ...
    print(f"\n{'='*70}")
    print("  Neural Network Surrogate Results (Dual-Hill CICR)")
    print(f"{'='*70}")
...
def generate_slurm_script(args):
    ...
    script_path = os.path.join(BASE_DIR, "new_fitting", "submit_fitting_g_cicr.sh")
    output_dir = os.path.join(BASE_DIR, "new_fitting", "fitting_results")

    script = f"""#!/bin/bash
#SBATCH --job-name=fit_g_cicr
#SBATCH --account=ctb-emuller
#SBATCH --time=12:00:00
#SBATCH --mem=32G
#SBATCH --cpus-per-task=8
#SBATCH --output={output_dir}/fit_g_cicr_%j.out
#SBATCH --error={output_dir}/fit_g_cicr_%j.err

source {BASE_DIR}/setupenv.sh
mkdir -p {output_dir}

SCRIPT="{BASE_DIR}/new_fitting/fit_params_g_CICR.py"

echo "Starting Dual-Hill CICR parameter fitting at $(date)"
echo "============================================"

# Run all 4 optimizers sequentially (each uses JIT-compiled ODE)
echo "\\n--- Differential Evolution ---"
python $SCRIPT --method de --max-iter 1000 --workers 8 \\
    --output {output_dir}/result_g_cicr_de.json

echo "\\n--- CMA-ES ---"
python $SCRIPT --method cmaes --max-iter 1000 \\
    --output {output_dir}/result_g_cicr_cmaes.json

echo "\\n--- Optuna ---"
python $SCRIPT --method optuna --n-trials 5000 \\
    --output {output_dir}/result_g_cicr_optuna.json

echo "\\n--- L-BFGS-B ---"
python $SCRIPT --method lbfgsb --n-starts 100 \\
    --output {output_dir}/result_g_cicr_lbfgsb.json
...
def main():
    parser = argparse.ArgumentParser(
        description="Fit Graupner-Brunel plasticity parameters with Dual-Hill CICR",
    ...
    # 2. Warmup numba JIT (updated for 12 Dual-Hill CICR parameters)
    ...
    _apply_cicr_batch(
        ...
        800.0, 2.5, 3.0, 0.005, 0.005, 0.005, 0.005, 1.9565, 0.5, 0.05, 0.0005, 3.0,
    )
    ...
    if output_path is None and len(methods) == 1:
        ...
        output_path = os.path.join(BASE_DIR, "new_fitting", "fitting_results", "all_results_g_cicr.json")
