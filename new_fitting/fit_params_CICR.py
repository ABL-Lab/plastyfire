...
FIT_PARAMS = [
    ...
    ("a30", 1.0, 10.0),
    ("a31", 1.0, 5.0),
    # Primed CICR parameters
    ("tau_IP3", 100.0, 5000.0),            # IP3 priming integrator decay time constant (ms)
    ("IP3_threshold", 0.5, 20.0),          # priming level that activates the coincidence gate
    ...
    ("a_er", 0.0, 0.05),                   # cpre coeff for SERCA activation threshold (mM)
    ("b_er", 0.0, 0.05),                   # cpost coeff for SERCA activation threshold (mM)
    ("g_serca_cicr", 0.01, 50.0),          # SERCA pump rate (mM/ms)
    ("g_release_cicr", 0.001, 10.0),       # max CICR release rate (1/ms)
    ("sat_cap", 0.001, 0.5),               # saturation cap for priming event amplitude (mM)
    ("sigmoid_slope", 1.0, 20.0),          # sigmoid gate steepness (coincidence detector sharpness)
    ("K_act", 0.0001, 0.005),              # RyR cytosolic Ca half-activation constant (mM, ~0.5 µM)
]
...
DEFAULT_PARAMS = {
    ...
    "a20": 1.127, "a21": 2.456,
    "a30": 5.236, "a31": 1.782,
    # Primed CICR defaults
    "tau_IP3": 500.0,        # IP3 priming decay (ms)
    "IP3_threshold": 3.0,    # priming level to activate sigmoid gate
    "a_evt": 0.003,   # ca_event_thresh = a_evt*Cpre + b_evt*Cpost; ~0.0003 mM for Cpre~0.09
    "b_evt": 0.003,
    "a_er": 0.005,    # gamma_er = a_er*Cpre + b_er*Cpost (SERCA activation threshold)
    "b_er": 0.005,
    "g_serca_cicr": 1.9565,
    "g_release_cicr": 0.5,
    "sat_cap": 0.05,         # 50 µM saturation cap on per-event priming amplitude
    "sigmoid_slope": 5.0,    # sigmoid gate steepness
    "K_act": 0.0005,         # 0.5 µM RyR cytosolic Ca half-activation
}
...
def _apply_cicr_batch(cai, t, c_pre, c_post,
                      ...
                      max_dt_sub=0.1):
    """Apply Primed CICR (Calcium-Induced Calcium Release) to calcium traces.

    Model (per synapse) — "Two-Key Vault":

      Key 1 (Slow, Presynaptic Activity History — IP3 Priming):
        * Rising-edge events detected when cai crosses ca_event_thresh.
        * Event amplitude = min(peak_cai - ca_event_thresh, sat_cap)
          (Option A: peak-tracking with saturation so bAPs don't super-prime).
        * Priming integrator: dP/dt = -P/tau_IP3 + event_amp (leaky bucket).

      Key 2 (Fast, Instantaneous Cytosolic Calcium — RyR Trigger):
        * Sigmoid gate: P_gate = sigmoid(sigmoid_slope * (Priming - IP3_threshold))
        * J_CICR = g_release * P_gate * (ca_cyt / (ca_cyt + K_act)) * (ca_er - ca_cyt)
          - The (ca_cyt/(ca_cyt+K_act)) term gates on bAP-level Ca.
          - Without Key 1: P_gate ≈ 0, J_CICR negligible.
          - Without Key 2 (no bAP): ca_cyt low, J_CICR negligible.
          - With both: large Ca tail released from ER, drives LTD.

      SERCA: J_serca (Hill kinetics on ca_cicr, guarded by gamma_er threshold).
      Conservation: fc*ca_cyt + fe*ca_er ≈ caAvg.

    Args:
        sat_cap: saturation cap for per-event priming amplitude (mM).
        sigmoid_slope: steepness of sigmoid coincidence detector gate.
        K_act: RyR cytosolic Ca half-activation constant (mM, ~0.5 µM).
    ...
    """
    K_serca = 0.1       # uM, fixed (SERCA half-activation)
    fc = 0.83
    fe = 0.17
    caAvg = 0.0017      # mM
    ...
    for s in range(n_syn):
        ...
        ca_cicr = 0.0
        was_above = 0
        event_peak_s = 0.0  # peak cai observed while above ca_event_thresh
        ...
        for i in range(n_time - 1):
            ...
            if is_above == 1 and was_above == 0:
                # Rising edge: begin tracking peak
                event_peak_s = cai[s, i]
            elif is_above == 1:
                # Still above threshold: update peak
                if cai[s, i] > event_peak_s:
                    event_peak_s = cai[s, i]
            elif was_above == 1:
                # Falling edge: fire normalized amplitude into priming bucket.
                # Normalising by sat_cap makes Priming_val dimensionless [0-N],
                # so IP3_threshold stays in the same [0.5-20] range as before.
                # Peaks below sat_cap contribute fractionally; peaks above = 1.0.
                raw_amp = event_peak_s - ca_event_thresh_s
                event_amp = (raw_amp / sat_cap) if raw_amp < sat_cap else 1.0
                event_peak_s = 0.0
            was_above = is_above

            # --- IP3 priming integrator (leaky bucket, Key 1) ---
            Priming_val = Priming_val * np.exp(-dt_full / tau_IP3) + event_amp

            # --- Sigmoid coincidence gate (smooth switch for Key 1) ---
            ...
            for _ in range(n_sub):
                ca_cyt = cai[s, i] + ca_cicr  # total cytosolic Ca

                # SERCA: operates only on ca_cicr, NOT on ca_cyt.
                # Reason: the NEURON cai_CR trace already contains its own SERCA.
                # Using ca_cyt would double-count the base pump.
                ...
                # J_CICR: Primed Coincidence Detection (Two-Key logic).
                #   P_gate             = Key 1 (is ER primed by IP3?)
                #   ca_cyt/(ca_cyt+K_act) = Key 2 (is cytosolic Ca high enough for RyR?)
                #   (ca_er - ca_cyt)   = driving force (release only when ER > cytoplasm)
                if ca_er > ca_cyt:
...
def objective(x):
    ...
    Parameter vector layout (21 params):
    ...
    for p_idx, (proto, target_ratio) in enumerate(_TARGETS.items()):
        ...
        for pd_item in _PROTOCOL_DATA[proto]:
            # Apply Primed CICR to raw cai -> cai_total
...
def _compute_ratios_cicr(x):
    """Forward model: params (21,) -> mean EPSP ratios (5,) via Primed CICR pipeline.
