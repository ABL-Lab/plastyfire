#!/bin/bash
# Submit BCL prefire simulations with gen-30 best params on NEW ION CHANNELS.
# 100 pairs × 7 protocols = 700 jobs.
#
# Same params as submit_optimizer_best_gen30_700.sh (hash d157569160e0) but:
#   - circuit_config = dhuruva_modified_ion_channels_circuit_config.json
#   - cpre_cpost_cache = ion_channels_tau278.pkl  (recomputed with new HOC)
#   - param_hash = d157569160e0_ic  (separate pkl, no collision with original)
#
# Output per workdir: simulation_edges_d157569160e0_ic.pkl
# Used by plot_stdp_ion_channels.py to produce STDP curve vs in vitro.
#
# Usage:  bash submit_ion_channels_gen30_700.sh [--dry-run]
...
for pair_dir in "$RESULTS_DIR"/*/; do
    ...
    for proto in "${PROTOCOLS[@]}"; do
        ...
        job_name="ic30_${proto}_${pair}"
        log_file="ic30_${proto}_${pair}.log"
